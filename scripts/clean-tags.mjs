...
// 2. Load manifest ground truth from all subjects if manifests exist
const MANIFEST_FILES = [
    ...
    'data/language_classification.json'
];
...
function getCanonicalTag(note, defaultPrefix = '') {
    ...
    // Search for any existing structured tag: e.g. physics::*, bangla::*, english::*, math::*, ict::*, chemistry::*, lang::*
    const match = tags.find(t => t.startsWith(defaultPrefix) && (t.endsWith('::concept') || t.endsWith('::mcq') || t.endsWith('::problem') || t.endsWith('::peg') || t.endsWith('::mining') || t.endsWith('::spelling') || t.includes('::cq::')));
...
async function main() {
    ...
    console.log('=== Cleaning Tags Across All Academic & Language Decks ===\n');
    ...
    const language = await cleanDeck('[🗣️] Language::[🔠] English', getCanonicalTag, 'lang');
    ...
    console.log(`[Language]    Total Notes: ${language.totalNotes.toString().padStart(3)} | Notes to strip: ${language.removals.length.toString().padStart(3)} | Canonical tags to add: ${language.additions.length.toString().padStart(3)}`);
    ...
    const allNotes = [...ict.notes, ...math.notes, ...bangla.notes, ...english.notes, ...physics.notes, ...chemistry.notes, ...language.notes];
    ...
    const allRemovals = [...ict.removals, ...math.removals, ...bangla.removals, ...english.removals, ...physics.removals, ...chemistry.removals, ...language.removals];
    const removeTagMap = {};
    ...
    const allAdditions = [...ict.additions, ...math.additions, ...bangla.additions, ...english.additions, ...physics.additions, ...chemistry.additions, ...language.additions];
    const addGroups = {};
